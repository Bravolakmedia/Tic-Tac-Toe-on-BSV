...
export class TicTacToe extends SmartContract {
    ...
    public move(n: bigint, sig: Sig) {
        ...
        assert(n >= 0n && n < 9n)
        // check signature `sig`
        const player: PubKey = this.isAliceTurn ? this.alice : this.bob
        ...
        assert(
            ...
            `board at position ${n} is not empty: ${this.board[Number(n)]}`
        )
        const play = this.isAliceTurn ? TicTacToe.ALICE : TicTacToe.BOB
        ...
        if (this.won(play)) {
            ...
            const bobOutput = Utils.buildPublicKeyHashOutput(
                hash160(this.bob),
                halfAmount
            )
